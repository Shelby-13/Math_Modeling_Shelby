//nvcc HW19Molecules1.cu -o bounce -lglut -lm -lGLU -lGL																													
...
void setInitialConditions()
{
	...
	int test;
	int tryCount;
	float globeMicrometerSize, angle1, angle2, radius;
	...
	massOfMolecule = 0.175*PI; // pg
	diameterOfMolecule = 1.0; // micron
	densityOfMolecule = 1.05; // pg/micron^3
	volumeOfMolecule = (PI/6.0)*diameterOfMolecule*diameterOfMolecule*diameterOfMolecule;
	
	printf("\n Mass of Molecule = %e picograms", massOfMolecule);
	printf("\n Diameter of Molecule = %e micrometers", diameterOfMolecule);
	printf("\n Density of Molecule = %e picograms/micrometer^3", densityOfMolecule);
	printf("\n Volume of Molecule = %e micrometers^3", volumeOfMolecule);
	printf("\n");
	...
	for(int i = 0; i < NUMBER_OF_BODIES; i++)
	{
		...
		while(test == 0)
		{
			...
			radius = ((float)rand()/(float)RAND_MAX)*globeMicrometerSize;
			...
			for(int j = 0; j < i; j++)
			{
				...
				if(seperation < (diameterOfMolecule))
				{
					test = 0;
					break;
				}
		...
		Color[i].x = ((float)rand()/(float)RAND_MAX)*0.15 + 0.40;
		Color[i].y = ((float)rand()/(float)RAND_MAX)*0.15 + 0.25;
		Color[i].z = ((float)rand()/(float)RAND_MAX)*0.15+0.30;
...
void drawPicture()
{
	...
	for(int i = 0; i < NUMBER_OF_BODIES; i++)
	{
		...
		glPushMatrix();
			...
			glutSolidSphere(diameterOfMolecule/2, 30, 30);
		glPopMatrix();
	}
	
	// Drawing the wall.
	// glBegin(GL_QUADS); 
	// 	glColor3d(0.482, 0.267, 0.91);
	// 	glVertex3f(25, -5, -5);
	// 	glColor3d(0.267, 0.91, 0.863);
	// 	glVertex3f(25, 5, -5);
	// 	glColor3d(0.267, 0.91, 0.863);
	// 	glVertex3f(25, 5, 5);
	// 	glColor3d(0.482, 0.267, 0.91);
...
float4 centerOfMass()
{
	...
	for(int i = 0; i < NUMBER_OF_BODIES; i++)
	{
    		centerOfMass.x += Position[i].x*massOfMolecule;
		centerOfMass.y += Position[i].y*massOfMolecule;
		centerOfMass.z += Position[i].z*massOfMolecule;
		totalMass += massOfMolecule;
	}
...
float4 linearVelocity()
{
	...
	for(int i = 0; i < NUMBER_OF_BODIES; i++)
	{
    		linearVelocity.x += Velocity[i].x*massOfMolecule;
		linearVelocity.y += Velocity[i].y*massOfMolecule;
		linearVelocity.z += Velocity[i].z*massOfMolecule;
		totalMass += massOfMolecule;
	}
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < (diameterOfMolecule))
			{
				...
				// if(BodyRadius[j] < BodyRadius[i])		//r1=larger and r2=smaller
				// {
				// 	r1 = BodyRadius[i];
				// 	r2 = BodyRadius[j];
				// }
				...
				temp = (r2*r2 - r1*r1 + d.w*d.w)/(2*d.w);		//Equivalent to 'x' from written homework calculations
				intersectionArea = PI * ((diameterOfMolecule*diameterOfMolecule)/4 - temp*temp);
				// if(0.0 < temp)
				// {
...
void updatePositions()
{
	for(int i = 0; i < NUMBER_OF_BODIES; i++)
	{
		...
		if(RunTime == 0.0)
		{
			Velocity[i].x += (Force[i].x/massOfMolecule)*(Dt/2.0);
			Velocity[i].y += (Force[i].y/massOfMolecule)*(Dt/2.0);
			Velocity[i].z += (Force[i].z/massOfMolecule)*(Dt/2.0);
		}
		else
		{
			Velocity[i].x += (Force[i].x/massOfMolecule)*Dt;
			Velocity[i].y += (Force[i].y/massOfMolecule)*Dt;
			Velocity[i].z += (Force[i].z/massOfMolecule)*Dt;
		}
