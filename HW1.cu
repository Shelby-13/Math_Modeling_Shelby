...
void getForces()
{
	...
	{
		Force.x = -(Position.x + sphereRadius - halfside) * k;
	}
	
	if((Position.x - sphereRadius) <= - halfside)
	{
		Force.x = (halfside - Position.x - sphereRadius) * k;
	}
	
	//check y position
	if((Position.y + sphereRadius) >= halfside)
	{
		Force.y = -(Position.y + sphereRadius - halfside) * k;
	}
	
	if((Position.y - sphereRadius) <= - halfside)
	{
		Force.y = (halfside - Position.y - sphereRadius) * k;
	}
	
	//check z position
	if((Position.z + sphereRadius) >= halfside)
	{
		Force.z = -(Position.z + sphereRadius - halfside) * k;
	}
	
	if((Position.z - sphereRadius) <= - halfside)
	{
		Force.z = (halfside - Position.z - sphereRadius) * k;
	}
}
