...
void getForces()
{
	...
	//check x position
	if((Position.x + sphereRadius) >= halfside)
	{
		Force.x = (SphereMass) * -a;
		Velocity.x -= (SphereMass * Velocity.x) / (SphereMass + boxMass);
	}
	...
	//check y position
	if((Position.y + sphereRadius) >= halfside)
	{
		Force.y = (SphereMass) * -a;
		Velocity.y -= (SphereMass * Velocity.y) / (SphereMass + boxMass);
	}
	...
	//check z position
	if((Position.z + sphereRadius) >= halfside)
	{
		Force.z = (SphereMass) * -a;
		Velocity.z -= (SphereMass * Velocity.z) / (SphereMass + boxMass);
	}
