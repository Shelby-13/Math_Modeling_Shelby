...
void getForces()
{
	...
	{	
		...
		{
			if((Position[i].x + sphereRadius) > 25.0 && (Position[i].x + sphereRadius) < 25.5) //have to add additional condition to account for balls that end up behind the wall from the side
			{
				amountOut = (Position[i].x + sphereRadius) - 25.0;
				if(Velocity[i].x < 0.0) kWall = kOut;
				else kWall = kIn;
...
void terminalPrint()
{
	...
	*/
	...
	// Tell people about your new key
		printf("\n s: Will shoot the system at the wall.");
	...
	printf("\n\n Allow asteroids to come together, zero out the system, then shoot them at the wall!");
