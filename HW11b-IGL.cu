//nvcc HW11b-IGL.cu -o bounce -lglut -lm -lGLU -lGL																													
...
{
	float inOut;
	float cSphere,cSphereReduction;		//equivalent to the kSphere and kSphereReduction for linear force model
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < SphereDiameter)
			{
				...
				// Quadratic model of force
				if(inOut < 0.0) magnitude = (cSphere * intersectionArea)/((2*SphereDiameter - d.w)*(SphereDiameter + d.w));					// If inOut is negative the sphere are converging.
				else magnitude = cSphereReduction * (cSphere * intersectionArea)/((2*SphereDiameter - d.w) * (SphereDiameter + d.w));		// If inOut is positive the sphere are diverging.
...
void terminalPrint()
{
	...
	*/
	...
	printf("\n");
	printf("\n 0: Will zero out the center of mass and linear velocity of the system.");
