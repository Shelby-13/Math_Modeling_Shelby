...
void setInitailConditions()
{
	...
	float seperation;
	int test;
	float convertedBoxSideLength;
	float convertedMaxVelocity;
	...
	MassUnitConverter = 9.39E20; // kg
	LengthUnitConverter = 940; // km
	TimeUnitConverter = 1.011111; // hr
	...
	SphereDiameter = 1.0; 					// Sphere Diameter is 1 of our length units
	sphereRadius = SphereDiameter/2.0;		// Sphere radius is 0.5 our length units
	SphereMass = 1.0;						// Sphere mass is 1 of our mass units
	BoxSideLength = 5.0;					// Box length is 5 times our length units
	MaxVelocity = 10.0;						// Velocity is 10 times our length units divided by our time units
	halfBoxSideLength = BoxSideLength/2.0;	// half the box side length is 0.5 the box side length in our length units
	
	// ??????????????????????????????????????????????????????????
	// Print out how many kilometers long each box side is.
	// Print out how many kilometers/hour the max Velocity is.
	// Uncomment these and fix them.
	convertedBoxSideLength = BoxSideLength * LengthUnitConverter;
	convertedMaxVelocity = MaxVelocity * LengthUnitConverter / TimeUnitConverter;

	printf("\n Box side length = %f kilometers", convertedBoxSideLength);
	printf("\n Max velocity = %f kilometers/hour", convertedMaxVelocity);
	...
	for(int i = 0; i < NUMBER_OF_BALLS; i++)
	{
		...
		for(int j = 0; j < i; j++)
		{
			...
			// Add gravity between asteroids here.
				Force[i].x += (GravityConstant*SphereMass*SphereMass) / (d*d);
				Force[i].y += (GravityConstant*SphereMass*SphereMass) / (d*d);
				Force[i].z += (GravityConstant*SphereMass*SphereMass) / (d*d);

				Force[j].x -= (GravityConstant*SphereMass*SphereMass) / (d*d);
				Force[j].y -= (GravityConstant*SphereMass*SphereMass) / (d*d);
				Force[j].z -= (GravityConstant*SphereMass*SphereMass) / (d*d);
...
{	
	...
	convertedTime = RunTime * TimeUnitConverter;
	printf("\n Time = %f hours", convertedTime );
	RunTime += Dt;
