	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			if(d.w < BodyRadius[i]+BodyRadius[j])
			{
				...
				float r = (BodyRadius[j]*BodyRadius[j] - BodyRadius[i]*BodyRadius[i] + d.w*d.w)/(2*d.w);
				intersectionArea = PI*(BodyRadius[j]*BodyRadius[j] - r*r);
				
				if (BodyRadius[i] < BodyRadius[j]) smallerBodyRadius = BodyRadius[i];
				else smallerBodyRadius = BodyRadius[j];

				if (r >= smallerBodyRadius) r = smallerBodyRadius;
