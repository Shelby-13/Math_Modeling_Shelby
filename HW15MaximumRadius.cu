//nvcc HW15Fix.cu -o bounce -lglut -lm -lGLU -lGL																													
...
void setInitialConditions()
{
	...
	float seperation;
	int test;
	float globeSize, angle1, angle2, radius, RN, TotRN, TotBodyMass, TotBodyRadius;
	double massOfCeres;
	...
	for (int i = 0; i < NUMBER_OF_BODIES; i++)
	{
		...
		//BodyMass[i] is temporary place holder
	...
	for (int i = 0; i < NUMBER_OF_BODIES; i++)
	{
		BodyMass[i] = massOfCeres * (BodyMass[i]/TotRN);
		TotBodyMass += BodyMass [i];
	}
	
	// ??? Set you mass unit
	MassUnitConverter = TotBodyMass/NUMBER_OF_BODIES; // kg
	
	// From the random masses you just did set all your corresponding radii
	TotBodyRadius = 0;
	for (int i = 0; i < NUMBER_OF_BODIES; i++)
	{
		BodyRadius[i] = diameterOfCeres * cbrt(BodyMass[i]/massOfCeres)/2.0;			//cbrt = cube root
		TotBodyRadius += BodyRadius[i];
	}

	// ?????? Set your length unit
	LengthUnitConverter = 2.0 * TotBodyRadius / NUMBER_OF_BODIES; // km
	TimeUnitConverter = sqrt(LengthUnitConverter*LengthUnitConverter*LengthUnitConverter/(G*MassUnitConverter)); // hr
	
	//Convert generated masses and radii to Simmulation Units
	for (int i = 0; i < NUMBER_OF_BODIES; i++)
	{
...
void getForces()
{
	...
	float r;
	float smallerBodyRadius;
	//float sphereRadius = BodyRadius;
	...
	{	
		...
		for(int j = 0; j < i; j++)
		{
			...
			kSphere = 1000 * (BodyRadius[j] + BodyRadius[i]);
			...
			if(d.w < BodyRadius[i]+BodyRadius[j])
			{
				...
				if (BodyRadius[i] < BodyRadius[j]) smallerBodyRadius = BodyRadius[i];
				else smallerBodyRadius = BodyRadius[j];

				r = (BodyRadius[j]*BodyRadius[j] - BodyRadius[i]*BodyRadius[i] + d.w*d.w)/(2*d.w);
				
				if (r >= smallerBodyRadius) intersectionArea = PI * smallerBodyRadius*smallerBodyRadius;
				else intersectionArea = PI*r*r;
