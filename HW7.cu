	...
	for(int i = 0; i < NUMBER_OF_BALLS; i++)
	{
		...
		while(test == 0)
		{
        	...
        	// Convert spherical coordinates to Cartesian coordinates
         	Position[i].x = randomNumber * sin(angle2) * cos(angle1);
         	Position[i].y = randomNumber * sin(angle2) * sin(angle1);
         	Position[i].z = randomNumber * cos(angle2);
			...
			// Color of each asteroid made random because all one color is no fun
			randomNumber = ((float)rand()/(float)RAND_MAX)*0.15 + 0.40;
			Color[i].x = randomNumber;
			randomNumber = ((float)rand()/(float)RAND_MAX)*0.15 + 0.25;
			Color[i].y = randomNumber;
			randomNumber = ((float)rand()/(float)RAND_MAX)*0.15+0.30;
			Color[i].z = randomNumber;
		
			//	All astroids start with no forces acting on them
			Force[i].x = 0.0;
			Force[i].y = 0.0;
			Force[i].z = 0.0;
		}
