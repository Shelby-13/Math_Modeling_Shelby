//nvcc HW7.cu -o bounce -lglut -lm -lGLU -lGL																													
...
void setInitailConditions()
{
	...
	for(int i = 0; i < NUMBER_OF_BALLS; i++)
	{
		...
		while(test == 0)
		{
		// Get random position in spherical coordinates
		randomNumber = ((float)rand()/(float)RAND_MAX)*masSphereSize;
		angle1 = ((float)rand() / RAND_MAX) * 2.0f * PI; 	// Range of 0->2pi (longitude)
        angle2 = ((float)rand() / RAND_MAX) * PI;       	// Range of 0->pi (latitude)
        
        // Convert spherical coordinates to Cartesian coordinates
         Position[i].x = maxSphereSize * sin(angle2) * cos(angle1);
         Position[i].y = maxSphereSize * sin(angle2) * sin(angle1);
         Position[i].z = maxSphereSize * cos(angle2);

		// Check if the position is within the sphere
        if (sqrt(Position[i].x*Position[i].x + Position[i].y*Position[i].y + Position[i].z*Position[i].z) <= maxSphereSize) // Check distance from all previously placed asteroids
		{
        	test = 1;
            for(int j = 0; j < i; j++) 
			{
                separation = sqrt((Position[i].x - Position[j].x)*(Position[i].x - Position[j].x) + (Position[i].y - Position[j].y)*(Position[i].y - Position[j].y) + (Position[i].z - Position[j].z)*(Position[i].z - Position[j].z));
                if(separation < SphereDiameter+0.1) 
				{
                    test = 0; // Position is too close to another asteroid
                    break;
                }
            }
		}
...
void getForces()
{
	...
	// If it is print out a note to make your repulsion stronger and termenate the program.
	float inOut;
	...
	{
		kSphere = 1000.0;
		kSphereReduction = 0.1;

		Force[i].x = 0.0;
		Force[i].y = 0.0;
		Force[i].z = 0.0;

		for(int j = 0; j < i; j++)	// Check distance vector between 2 asteroids
		{
			...
			// Calculate the normal vector between 2 spheres
			normX = dx/d;
            normY = dy/d;
            normZ = dz/d;

            // If the relative velocity is positive, they are moving away from each other
            if(d < SphereDiameter) // Calculate the impulse scalar
			{
				// Checks to make sure the spheres are not too close to each other
				if(d < sphereRadius)
				{
        		...
        		dvz = Velocity[i].z - Velocity[j].z;

           		inOut = dvx*dx + dvy*dy + dvz*dz;

				magnitude = kSphere*(SphereDiameter - d);

				if(inOut > 0.0) magnitude = magnitude*kSphereReduction;

				// Doling out the force in the proper perfortions using unit vectors.
				Force[i].x -= magnitude*normX;
				Force[i].y -= magnitude*normY;
				Force[i].z -= magnitude*normZ;
				// A force on me causes the opposite force on you. 
				Force[j].x += magnitude*normX;
				Force[j].y += magnitude*normY;
				Force[j].z += magnitude*normZ;
			}
...
void terminalPrint()
{
	...
	*/
	...
	// Which keys to press to change the view
	printf("\n");
