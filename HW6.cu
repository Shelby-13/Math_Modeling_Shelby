...
void setInitailConditions()
{
	...
	for(int i = 0; i < NUMBER_OF_BALLS; i++)
	{
		...
		angle1 = ((float)rand() / RAND_MAX) * 2.0f * M_PI; 	// Range of 0->2pi (longitude)
        angle2 = ((float)rand() / RAND_MAX) * M_PI;       	// Range of 0->pi (latitude)
	...
	for(int i = 0; i < NUMBER_OF_BALLS; i++)
	{
		...
		for(int j = 0; j < i; j++)	// Check distance vector between 2 asteroids
		{
        	...
        	dvz = Velocity[i].z - Velocity[j].z;
           	relativeVelocity = dvx*normX + dvy*normY + dvz*normZ;
