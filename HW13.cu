...
void setInitailConditions()
{
	...
	massOfCeres = 9.838e20; // kg
	diameterOfCeres = 940; // km
	D3 = diameterOfCeres * diameterOfCeres * diameterOfCeres; //km^3
	volumeOfCeres = (PI/6) * D3; // km^3
	densityOfCeres = massOfCeres / volumeOfCeres; // kg/km^3
	
	MassUnitConverter = massOfCeres / NUMBER_OF_BODIES; // kg
	LengthUnitConverter = diameterOfCeres / cbrt(NUMBER_OF_BODIES); // km	cbrt = cube root
...
void drawPicture()
{
	...
	glBegin(GL_QUADS);
		...
		if(temp <= 255)
		{
			glColor3d((float)temp/253.0, 51.0, 55.0);
		}
		else 
		{
			temp = 256 - temp%255;
