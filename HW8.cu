...
void KeyPressed(unsigned char key, int x, int y)
{
	...
	if(key == '1')
	{
		...
		printf( "\nCenter of Mass:  %fx, %fy, %fz", pos.x, pos.y, pos.z );
		printf( "\nLinear Velocity: %fx, %fy, %fz\n", vel.x, vel.y, vel.z );
	}
...
float4 centerOfMass()
{
	...
	//divide the center of mass above by the total mass of all the balls
...
float4 linearVelocity()
{
	...
	//divide the linear velocity above by the total mass of all the balls
